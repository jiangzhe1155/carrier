  ...
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right" style="margin-top: 20px;margin-left: 10px">
      <el-breadcrumb-item v-for=" item in navigator">
        <el-button type="text" :disabled="item.isLast" @click="onClickNavigatorItem(item.relativePath)" size="medium">
          {{item.name}}
        </el-button>
      </el-breadcrumb-item>
    </el-breadcrumb>
    <el-table :data="files" stripe @selection-change="handleSelectionChange" ref="fileTable">
      ...
      <el-table-column prop="size" label="大小">
        <template slot-scope="scope">{{util.formatSize(scope.row.size)}}</template>
      </el-table-column>
