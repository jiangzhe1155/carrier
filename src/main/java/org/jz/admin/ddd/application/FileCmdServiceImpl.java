...
public class FileCmdServiceImpl implements FileCmdService {
    ...
    public Response rename(FileRenameCmd cmd) {
        ...
        if (fileRepository.getFileByRelativePath(file.getDescription().getRelativePath()) != null) {
            file.toNewFileName();
        }
        if (!file.isFolder()) {
    ...
    public Response checkUpProgress(FileCheckUpProgressCmd cmd) {
        ...
        TFileResource fileStoreDO = fileResourceRepository.getResourceByIdentifier(cmd.getIdentifier());
        if (fileStoreDO != null) {
    ...
    public Response list(FileListQry qry) {
        ...
        Page<TFile> filePage = fileRepository.filePage(parentFolder.getId(), qry.getType(),
